    ...
    <main className="app-shell">
      ...
      <section className={`content-shell ${styles.workspace}`}>
        ...
        <section className={`panel ${styles.panel}`}>
          <div className={`card-head ${styles.header}`}><div><h2>Evidencias y aprobaciones</h2><p>{workspace.evidence.length} adjuntos registrados sobre {workspace.activities.length} productos visibles.</p></div><div className="actions"><button className="icon-button" type="button" aria-label="Adjuntar nueva evidencia" disabled={!canAttach} onClick={() => setUploadActivityId("")}><Plus size={16} /></button><button className="button secondary" type="button" disabled={workspace.isRefreshing} onClick={() => void workspace.refresh().catch(() => undefined)}><RefreshCw size={16} /> {workspace.isRefreshing ? "Actualizando" : "Actualizar"}</button></div></div>
