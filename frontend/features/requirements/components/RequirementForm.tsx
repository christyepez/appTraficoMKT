  ...
  return (
    <RequirementDialog labelledBy="requirement-form-title" onClose={onCancel} closeDisabled={isSubmitting}>
      ...
      <form className="form" onSubmit={handleSubmit(submit)} noValidate>
        ...
        <Field idPrefix="requirement" errorClassName={styles.fieldError} label="Actividad o evento" error={errors.activityOrEvent?.message}><input disabled={!editable} {...register("activityOrEvent")} /></Field>
        <Field label="Nombre del solicitante" error={errors.requesterName?.message}><input disabled={!editable} {...register("requesterName")} /></Field>
        ...
        <Field label="Facultad" error={errors.facultyId?.message}><select disabled={!editable} {...register("facultyId", { onChange: () => setValue("careerId", "") })}><option value="">Seleccione…</option>{catalogs.faculties.map(option)}</select></Field>
        ...
        <Field label="Fecha y hora de inicio" error={errors.startAt?.message}><input disabled={!editable} type="datetime-local" {...register("startAt")} /></Field>
        ...
        <Field label="Público" error={errors.audienceType?.message}><select disabled={!editable} {...register("audienceType")}><option value="internal">Interno</option><option value="external">Externo</option><option value="mixed">Mixto</option></select></Field>
        ...
        <label className="field field-wide" htmlFor="requirement-attachments"><span>Adjuntos del requerimiento</span>
        ...
        <section className={styles.summary} aria-label="Resumen del requerimiento"><strong>Resumen</strong><span>{facultyId ? `${catalogs.faculties.find((item) => item.id === facultyId)?.name ?? ""} / ${careers.find((item) => item.id === careerId)?.name ?? ""}` : "Seleccione facultad y carrera."}</span></section>
