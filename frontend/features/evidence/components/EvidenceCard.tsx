import { ChevronDown, ChevronRight, Eye, FileText, Plus, Trash2 } from "lucide-react";
  ...
  return (
    <article className={`card compact-card ${styles.card}`}>
      <div className="card-head">
        ...
        <div className="card-meta"><span className="badge">{files.length} adjunto(s)</span><span className="badge">{decisions.length} aprobación(es)</span><button className="icon-button" type="button" aria-label={`Adjuntar evidencia a ${activity.productId}`} disabled={isFinalActivity(activity.status)} onClick={onAttach}><Plus size={16} /></button></div>
      </div>
