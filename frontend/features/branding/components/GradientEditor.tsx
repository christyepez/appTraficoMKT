...
export function GradientEditor({ kind, register }: { kind: "button" | "header" | "menu"; register: UseFormRegister<BrandFormValues> }) {
  ...
  return <fieldset className="field field-wide"><legend>Degradado de {fields.label}</legend><div className="form"><ColorField label="Color inicial" name={fields.base} register={register}/><ColorField label="Color final" name={fields.end} register={register}/><label className="field"><span>Dirección</span><select {...register(fields.direction)}><option value="135deg">Diagonal</option><option value="to right">Horizontal</option><option value="to bottom">Vertical</option></select></label><div className="check-group"><label className="check-field">Usar degradado<input type="checkbox" {...register(fields.enabled)}/></label></div></div></fieldset>;
}
