...
export function SatisfactionExperience({ token }: { token: string }) {
  ...
  return (
    <main className={accessStyles.page}>
      <section className={`${accessStyles.panel} ${accessStyles.satisfactionPanel}`}>
        <div className={`brand ${accessStyles.brand}`}><strong>Encuesta de satisfacción</strong><span>Universidad Indoamérica</span></div>
        ...
        {workspace.context && <div className={accessStyles.summary}><strong>{workspace.context.requirementCode}</strong><span>{workspace.context.activityOrEvent}</span></div>}
        {terminal && <div className={accessStyles.success} role="status"><CheckCircle2 size={32} /><p>{workspace.message || "Esta encuesta ya fue respondida. Gracias por su participación."}</p></div>}
